...
def normalize(text: str) -> str:
    ...
    for token in text.split():
        clean_token = re.sub(r"[^\dא-ת]", "", token)
        if not clean_token:
            continue
        if clean_token.isdigit():
            normalized_tokens.append(str(int(clean_token)))
            continue
        if clean_token in SINGLE_NUMBER_WORDS:
            normalized_tokens.append(SINGLE_NUMBER_WORDS[clean_token])
            continue

        reduced_token = clean_token
        while len(reduced_token) > 1 and reduced_token[0] in NUMBER_PREFIXES and reduced_token not in SINGLE_NUMBER_WORDS:
            candidate = reduced_token[1:]
            if candidate in SINGLE_NUMBER_WORDS:
                reduced_token = candidate
                break
            reduced_token = candidate

        if reduced_token in SINGLE_NUMBER_WORDS:
            normalized_tokens.append(SINGLE_NUMBER_WORDS[reduced_token])
            continue

        normalized_tokens.append(ORTHOGRAPHIC_VARIANTS.get(clean_token, clean_token))
